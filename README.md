...
- The reward system is very bad than Mario can't completed this stage: Mario still earn reward when he move in the loop (incorrect paths). I solved it same as stage 4-4 and 7-4.
...
- Mario need find hidden brick to complete this stage:
  ...
  - If Mario goes right through the water pipe, Mario will enter a looped road (incorrect paths)
...
- First, I find optimal hyperparameters for stages 8-4 (I do this project just to complete this stage because normal PPO wins 31/32 other stages):
  ...
  - I don’t change epoch = 10, lambda = 0.95, learning_rate = 7e-5, target_kl = 0.05, clip_param = 0.05, max_grad_norm = 0.5, norm_adv = False, V_coef = 0.5 and loss_type = ‘mse’ (because I think it is best when tuning normal PPO, just my bias experiment):
    - update: I find that 'huber' actually better than 'mse'. In PPO, I tuning loss_type same time with other hyperparameters than I mistaken that 'mse' better than 'huber' (I realize this when training with new project).
...
- Noted:
  - RL is very sensitive to hyperparameters, and some hyperparameters work with these stages but don’t work with other stages. Then we need custom hyperparameters for some hard stages like 5-3, 7-2, 8-1, 8-4. I don’t have enough time and resources to find optimal hyperparameters that can complete all stages (or this is imposible :)) ).
  - I use min-max scaling for instrinsic reward because running mean std not working with me (because I use poor init as I explain in **Question** session).
...
* How can you improve this code?
  ...
  - Update: You can try better init weights for model:
    - In this project, I didn't initialize weights for models because I didn't see the impact of initializing weights in older projects (PPO, A2C). Now, I'm trying a new project, and I find that initializing weights helps a lot, especially with RND.
    - Because target network is just random, I find that poor initialization of weights will yield a very small RND_loss, and intrinsic reward will also be very small. I observed this in this project, but at that time, I thought it was due to the running mean and standard deviation scaling yielding poor loss instead of poor initialization. I found that using min-max scaling solved this problem. But now, I realize that good initialization solves it even better.
  - Update: You can try scaling reward by divide the reward by running std of returns.
